      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {overviewCards.map((card, index) => (
            <div key={index} className={`p-6 rounded-2xl ${card.bgColor} border ${card.borderColor}`}>
              ...
              <ul className="space-y-3 text-neutral-600">
                {card.items.map((item, itemIndex) => (
                  <li key={itemIndex} className="flex items-start gap-2">
                    <div className={`w-1.5 h-1.5 rounded-full mt-1.5 ${card.icon === 'Zap' ? 'bg-primary' : 'bg-neutral-400'}`}></div>
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
            ...
            <div key={index} className="p-6 border border-neutral-200 rounded-2xl">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 bg-primary/10 rounded-xl flex items-center justify-center relative">
                  ...
                  {feature.secondIcon && <feature.secondIcon className="w-5 h-5 text-primary absolute" />}
                </div>
